...
#a function to get a random number to place in the designated index list position as the computer's turn
def computer():
...
while(check == False):
    ...
    #check value
    check2 = False
    #this while loop iterates to generate a index position value from the random computer value. At the index position, there should be no X or O, and that index can only be registered as a position
    while(check2 == False):
        p2 = computer()
        if(row1[p2-1] == " "):
            check2 = True
